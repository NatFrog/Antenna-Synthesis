"""
Train the actual 2-to-4 fusion cGAN (UW sub-array sources).

Variant of train_cgan_2to4_fusion.py that consumes real UW sub-array data:
  ch0 = matlab_2x2     (real MATLAB sub-array, normalised with norm_stats_2x2_uw.npz)
  ch1 = hfss_2x2       (real HFSS  sub-array, normalised with norm_stats_2x2_uw.npz)
  ch2 = matlab_4x4     (normalised with norm_stats.npz)
  ch3 = dphase_x / 180 (broadcast scalar)
  ch4 = dphase_y / 180 (broadcast scalar)
  target = hfss_4x4    (normalised with norm_stats.npz)

The architecture (ResUNetGenerator + PatchDiscriminator) and all
hyperparameters are identical to train_cgan_2to4_fusion.py (the synthetic
variant) so the only difference between the two trained models is the data
on ch0 + ch1.

Usage:
    python -m scripts.train_cgan_2to4_fusion_uw
"""
...
from scripts.train_cgan_2to4_fusion import ResUNetGenerator  # reuse identical generator class

# ─── Paths ───────────────────────────────────────────────────────────────────
HDF5 = PROCESSED_DIR / "antenna_data_2to4_uw.h5"
NORM_2X2 = PROCESSED_DIR / "norm_stats_2x2_uw.npz"
NORM_4X4 = PROCESSED_DIR / "norm_stats.npz"
SPLITS = PROCESSED_DIR / "split_indices_2to4_uw.npz"
CKPT_DIR = CHECKPOINTS_DIR / "cgan_resunet_patchgan_2to4_uw"
CKPT_DIR.mkdir(parents=True, exist_ok=True)

# ─── Hyperparameters (identical to train_cgan_2to4_fusion.py) ────────────────
...
class FusionDatasetUW(Dataset):
    """HDF5-backed dataset for the actual fusion model. Same channel layout as
    the synthetic fusion dataset; only the underlying h5 dataset name for ch1
    differs (`hfss_2x2` for real HFSS sub-array, vs `hfss_pred_2x2` for
    cGAN-predicted)."""
    ...
    def __getitem__(self, i):
        ...
        h2 = f["hfss_2x2"][idx].astype(np.float32)        # real HFSS, not predicted
        ...
        h2_n = (h2 - self.mean_2x2) / self.std_2x2
        ...
        x = np.stack([m2_n, h2_n, m4_n, dphase_x, dphase_y], axis=0)  # (5, 181, 360)
        y = h4_n[None, ...]
...
def main():
    ...
    tr_ds = FusionDatasetUW(HDF5, tr, mean_2x2, std_2x2, mean_4x4, std_4x4)
    va_ds = FusionDatasetUW(HDF5, va, mean_2x2, std_2x2, mean_4x4, std_4x4)
