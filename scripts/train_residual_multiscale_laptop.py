"""
Laptop-oriented copy of ``train_residual_multiscale`` (same G/D/loss/datasets).

Differences
-----------
- **16x16 monitoring**: RMSE / checkpoint selection uses **all 200** hfss_16x16
  samples from the sorted 4->8 **test** fold (same set as the full
  ``matlab_16x16_test.npz`` pack). There is no separate 100/100 val vs held-out
  split inside training — selection and “test” fold coincide by design here.
- **Torch throughput** (when CUDA is available): TF32 for matmul/cuDNN,
  cuDNN benchmark, high matmul precision (no ``torch.compile`` on ``G`` to keep
  EMA checkpoint round-trips reliable).
- **DataLoader**: slightly larger batch for 16x16 eval only; light worker
  prefetch where safe on your OS.
- **Checkpoints**: ``checkpoints/residual_multiscale_with_anchor_laptop/`` so
  runs do not overwrite the default trainer.

Usage:
    python -m scripts.train_residual_multiscale_laptop
"""
...
CKPT_DIR = CHECKPOINTS_DIR / "residual_multiscale_with_anchor_laptop"
...
PLATEAU_PATIENCE = 4
PLATEAU_MIN_LR = 1e-6
# Light prefetch; set 0 if HDF5 + multiprocessing misbehaves on your machine.
NUM_WORKERS = min(2, max(0, (os.cpu_count() or 1) // 2))
...
# Fewer, larger eval batches for 16x16 (memory permitting).
VAL16_BATCH = min(32, max(BATCH, 8))


def _configure_torch_laptop() -> None:
...
class Val16x16Dataset(Dataset):
    """N hfss_16x16 samples (here N=200): matlab + hfss + hfss_pred_8x8 anchor."""

    def __init__(self, indices, mat16_arr, mean_mat, std_mat):
        self.indices = np.asarray(indices, dtype=np.int64)
        self.mat16 = mat16_arr.astype(np.float32)
        self.mean_mat = mean_mat.astype(np.float32)
        self.std_mat = np.maximum(std_mat.astype(np.float32), 1e-6)
...
def train_one_epoch(G, D, ema, loader, opt_g, opt_d, scaler_g, scaler_d, bce, recon_loss):
    G.train()
    D.train()
    g_losses = []
    d_losses = []
    ...
    for step, (x, target_dB, matlab_dB, null_mask) in enumerate(loader, 1):
        ...
        with torch.amp.autocast(device_type=AMP_DEVICE_TYPE, dtype=AMP_DTYPE, enabled=USE_AMP):
            delta_n = G(x)
            recon, l1_val, ssim_val, pred_dB = recon_loss(
                delta_n, matlab_dB, target_dB, null_mask
            )
        ...
        if step == 1 or step % STEP_PRINT_EVERY == 0 or step == total_steps:
            print(
                f"  step {step:03d}/{total_steps} | g={g_loss.item():.4f} d={d_loss.item():.4f}"
                f" l1_dB={l1_val.item():.3f} ssim={ssim_val.item():.4f}",
                flush=True,
            )
...
def evaluate_indist(G, ema, loader, recon_loss):
    ...
    try:
        ...
        for x, target_dB, matlab_dB, _null in loader:
            ...
            matlab_dB = matlab_dB.to(DEVICE)
            delta_n = G(x)
            pred_dB = recon_loss.compose(delta_n, matlab_dB)
            yt.append(target_dB.cpu().numpy())
            yp.append(pred_dB.cpu().numpy())
...
def main():
    ...
    print(
        f"[laptop] torch={torch.__version__} cuda={torch.version.cuda} "
        ...
        f"num_workers={NUM_WORKERS} val16_batch={VAL16_BATCH} ckpt={CKPT_DIR.name}",
        flush=True,
    )
    ...
    print(
        f"Loaded combined norm stats: mean range [{mean_mat.min():.2f}, {mean_mat.max():.2f}], "
    ...
    # All 200 test-betas: selection metric == full 16x16 fold (no 100/100 split).
    val16_idx = test48
    print(
        f"Train: N=4 {len(tr4)} + N=8 {len(tr8)} = {len(tr4) + len(tr8)} samples",
        flush=True,
    )
    print(
        f"In-dist val: N=4 {len(va4)} + N=8 {len(va8)} = {len(va4) + len(va8)} samples",
        flush=True,
    )
    print(
        f"16x16 monitor: ALL {len(val16_idx)} hfss_16x16 (sorted 4to8 test fold) — "
        f"selection uses full set",
        flush=True,
    )
    ...
    if len(val16_idx) != len(m16_arr):
        raise RuntimeError(
            f"16x16 monitor: len(idx)={len(val16_idx)} != matlab arr {len(m16_arr)}"
        )
    val16_ds = Val16x16Dataset(val16_idx, m16_arr, mean_mat, std_mat)
    val16_loader = DataLoader(
        val16_ds,
        batch_size=VAL16_BATCH,
        shuffle=False,
        num_workers=NUM_WORKERS,
        pin_memory=PIN_MEMORY,
        persistent_workers=NUM_WORKERS > 0,
    ...
    print(
        f"G params: {sum(p.numel() for p in G.parameters()) / 1e6:.2f}M  "
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            f"  train (N=4 + N=8 only): g_loss={g_avg:.4f}  d_loss={d_avg:.4f}  "
        ...
        if epoch % VALIDATE_EVERY == 0 or epoch == 1 or epoch == EPOCHS:
            ...
            print(
                f"  val_indist (N=4+N=8, EMA): rmse={m_indist['rmse']:.4f}  "
                f"mae={m_indist['mae']:.4f}  r={m_indist['pearson']:.4f}",
                flush=True,
            )

            m_16 = evaluate_16x16(G, ema, val16_loader, recon_loss)
            print(
                f"  16x16 ALL 200 (EMA, NO GRADIENTS): rmse={m_16['rmse']:.4f}  "
                f"mae={m_16['mae']:.4f}  r={m_16['pearson']:.4f}",
                flush=True,
            )
            ...
            if m_16["rmse"] < best_rmse_16 - 1e-5:
                ...
                print(
                    f"  * new best (16x16-all-200 rmse {best_rmse_16:.5f}) saved to "
                    f"best_generator.pt",
                    flush=True,
                )
            else:
                ...
                print(
                    f"  (no 16x16 improvement for {epochs_without_improve} epochs)",
                    flush=True,
                )
        ...
        print(
            f"  [time] epoch={epoch_dt / 60:.2f} min  "
        ...
        if epochs_without_improve >= EARLY_STOP_PATIENCE:
            print(
                ...
                f"(16x16-all-200 rmse {best_rmse_16:.5f})",
                flush=True,
            )
            break
    ...
    print(
        ...
        f"16x16-all-200 rmse {best_rmse_16:.5f}.  Checkpoints in {CKPT_DIR}",
        flush=True,
    )
