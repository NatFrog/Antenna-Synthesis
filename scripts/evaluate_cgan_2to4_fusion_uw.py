"""Evaluate the actual 2-to-4 fusion cGAN on the held-out test set.

Mirrors scripts/evaluate_cgan_2to4_fusion.py exactly; only the artefacts the
script consumes (HDF5, norm stats, splits, checkpoint) and the output dir
differ. Same metric set so the resulting metrics.txt can be diffed
1-to-1 against the synthetic fusion model's metrics.

Usage:
    python -m scripts.evaluate_cgan_2to4_fusion_uw
"""
...
from scripts.train_cgan_2to4_fusion import ResUNetGenerator
...
CKPT = CHECKPOINTS_DIR / "cgan_resunet_patchgan_2to4_uw" / "best_generator.pt"
RESULTS = RESULTS_DIR / "cgan_resunet_patchgan_2to4_uw"
...
def main():
    ...
    ds = FusionDatasetUW(HDF5, test_idx, mean_2x2, std_2x2, mean_4x4, std_4x4)
    ...
    print("\n=== Pattern-level metrics (actual fusion cGAN) ===")
    for k, v in pm.items():
        print(f"  {k}: {v:.6f}")
    ...
    # ── Baseline (4x4 MATLAB vs 4x4 HFSS, no correction) ──
    bm = compute_batch_metrics(matlab_4x4_raw, targets_db)
    print("\n=== Baseline (no correction) pattern metrics ===")
    for k, v in bm.items():
        print(f"  {k}: {v:.6f}")
    ...
    with open(RESULTS / "metrics.txt", "w") as f:
        f.write("Evaluation: cgan_resunet_patchgan_2to4_uw (actual fusion)\n")
    ...
    for idx in tqdm(vis_indices, desc="Saving plots"):
        plot_pattern_comparison(
            ...
            title=f"Actual fusion cGAN test sample {idx}",
    ...
    plot_error_distribution(
        errors, title="Actual fusion cGAN error distribution",
        save_path=str(RESULTS / "error_distribution.png"),
    )
    plot_scatter_pred_vs_true(
        preds_db, targets_db, title="Actual fusion cGAN: predicted vs true",
