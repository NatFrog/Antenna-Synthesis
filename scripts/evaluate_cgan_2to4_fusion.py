"""Evaluate the 2-to-4 fusion cGAN on the held-out test set.

Metrics (mirrors scripts/evaluate_cgan_2x2.py):
  - Pattern-level: RMSE, MAE, Pearson r, SSIM (if available)
  - Antenna-specific: peak direction, HPBW, SLL (matched-position)
  - Null-specific: RMSE at/away from nulls, depth error, fill accuracy

Baseline for comparison = matlab_4x4 patterns (no correction) vs hfss_4x4 target.

Usage:
    python -m scripts.evaluate_cgan_2to4_fusion
"""
...
def main():
    ...
    ds = FusionDataset(HDF5, test_idx, mean_2x2, std_2x2, mean_4x4, std_4x4)
    ...
    print("\n=== Pattern-level metrics (fusion cGAN) ===")
    for k, v in pm.items():
        print(f"  {k}: {v:.6f}")
    ...
    with open(RESULTS / "metrics.txt", "w") as f:
        f.write("Evaluation: cgan_resunet_patchgan_2to4 (fusion)\n")
    ...
    for idx in tqdm(vis_indices, desc="Saving plots"):
        plot_pattern_comparison(
            ...
            title=f"Synthetic fusion cGAN test sample {idx}",
    ...
    plot_error_distribution(
        errors, title="Synthetic fusion cGAN error distribution",
        save_path=str(RESULTS / "error_distribution.png"),
    )
    plot_scatter_pred_vs_true(
        preds_db, targets_db, title="Synthetic fusion cGAN: predicted vs true",
