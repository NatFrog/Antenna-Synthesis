"""Evaluate the enhanced no-matlab_4x4 ablation of the synthetic 2-to-4 fusion cGAN.

Mirrors scripts/evaluate_cgan_2to4_fusion.py exactly; the differences vs. the
full-input fusion evaluator are:
  * Dataset class (no matlab_4x4, with residual channel; returns null mask).
  * Wider + attention generator (EnhancedResUNetGenerator).
  * Loads EMA-weighted best_generator.pt.
  * Separate output dir.

Same metric set so the resulting metrics.txt can be diffed 1-to-1 against the
full-input fusion model's metrics.

Usage:
    python -m scripts.evaluate_cgan_2to4_fusion_no_m4
"""
...
from scripts.train_cgan_2to4_fusion_no_m4 import (
    FusionDatasetNoM4, EnhancedResUNetGenerator, GEN_BASE, ATTN_HEADS,
)
...
RESULTS = RESULTS_DIR / "cgan_resunet_patchgan_2to4_no_m4"
...
def main():
    ...
    ds = FusionDatasetNoM4(HDF5, test_idx, mean_2x2, std_2x2, mean_4x4, std_4x4,
                           augment_noise=False)
    ...
    print("\n=== Pattern-level metrics (no-m4 fusion cGAN) ===")
    for k, v in pm.items():
        print(f"  {k}: {v:.6f}")
    ...
    # ── Baseline (4x4 MATLAB vs 4x4 HFSS target, no correction) ──
    bm = compute_batch_metrics(matlab_4x4_raw, targets_db)
    print("\n=== Baseline (no correction, matlab_4x4 vs hfss_4x4) pattern metrics ===")
    for k, v in bm.items():
        print(f"  {k}: {v:.6f}")
    ...
    with open(RESULTS / "metrics.txt", "w") as f:
        f.write("Evaluation: cgan_resunet_patchgan_2to4_no_m4 (synthetic fusion, no matlab_4x4 channel)\n")
    ...
    for idx in tqdm(vis_indices, desc="Saving plots"):
        plot_pattern_comparison(
            ...
            title=f"No-m4 fusion cGAN test sample {idx}",
    ...
    plot_error_distribution(
        errors, title="No-m4 fusion cGAN error distribution",
        save_path=str(RESULTS / "error_distribution.png"),
    )
    plot_scatter_pred_vs_true(
        preds_db, targets_db, title="No-m4 fusion cGAN: predicted vs true",
